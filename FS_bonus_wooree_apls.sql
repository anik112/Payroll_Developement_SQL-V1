...
SELECT bamt.cardno, bamt.enmname_bangla, bamt.lineno, bamt.lineno_bangla, bamt.departmentnm,
               bamt.dept_bangla, bamt.sectionnm, bamt.sec_bangla, bamt.machineno, bamt.designation, bamt.designation_ban,
               info.joining_date,bamt.empname,bamt.jobday, bamt.jobperiod,
              (bamt.grosssal+bamt.grosssal_bk) gross_sal, 
              (bamt.basicsal+bamt.basicsal_bk) basic_sal, 
              (bamt.payable_ch+bamt.payable_bk) payable_amt,
              (bamt.payment_ch_rd+bamt.payment_bk_rd) net_payment_amt,
              bamt.calculation_status, info.salary_grade, SIGN.empsign
FROM   TB_BONUS_AMOUNT bamt,TB_PERSONAL_INFO info,  TB_PERSONAL_INFO_SIGN SIGN
WHERE  info.company    = :p_company
AND    info.company        = bamt.company
AND    info.company        = SIGN.company
AND    bamt.finyear          = :p_year
AND    bamt.finmonth       = :p_month
AND    bamt.payment_ch_rd   > 0
AND    info.cardno           = bamt.cardno
AND    info.cardno           = SIGN.cardno 
AND    bamt.departmentnm     LIKE DECODE(NVL(:p_dept,'all'),'all','%',:p_dept) 
AND    bamt.sectionnm            LIKE DECODE(NVL(:p_section,'all'),'all','%',:p_section) 
AND    info.workertype             LIKE DECODE(NVL(:p_worker,'all'),'all','%',:p_worker)
AND    bamt.designation          LIKE DECODE(NVL(:p_designation,'all'),'all','%',:p_designation) 
...
AND    bamt.finyear          = :p_year
AND    bamt.finmonth       = :p_month
AND    bamt.payment_ch_rd+bamt.payment_bk_rd   > 0
AND    info.cardno           = bamt.cardno
AND    bamt.departmentnm     LIKE DECODE(NVL(:p_dept,'all'),'all','%',:p_dept) 
AND    bamt.sectionnm            LIKE DECODE(NVL(:p_section,'all'),'all','%',:p_section) 
AND    info.workertype             LIKE DECODE(NVL(:p_worker,'all'),'all','%',:p_worker)
